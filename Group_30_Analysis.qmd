...
g <-
  ...
  ylab("Age") +
  theme_minimal()
save_fig('age', plot = g + ggtitle('Age vs. Income'))
g
```
...
```{r}
#| fig-cap: Working Hours vs. Income
#| fig-align: center
...
g <-
  ...
  ylab("Hours per Week") +
  theme_minimal()
save_fig('hour', plot = g + ggtitle("Working Hours vs. Income"))
g
```
...
```{r}
#| fig-cap: Education Level vs. Income
#| fig-align: center
...
g <-
  ...
  ylab("Proportion") +
  theme_minimal()
save_fig('education', plot = g + ggtitle("Education Level vs. Income"))
g
```
...
```{r}
#| fig-cap: Marital Status vs. Income
#| fig-align: center

g <-
  ggplot(df, aes(x = Marital_Status, fill = as.factor(Income))) +
  geom_bar(position = "fill") +
  coord_flip() +
  scale_fill_manual(values = c("skyblue", "orange"),
                    labels = c("<=50K", ">50K")) +
  ...
  ylab("Proportion") +
  theme_minimal()
save_fig('marital status', plot = g + ggtitle("Marital Status vs. Income"))
g
```
...
g <-
  ...
  ylab("Proportion") +
  theme_minimal()
save_fig('occupation', plot = g + ggtitle("Occupation vs. Income"))
g
```
...
g <-
  ...
  ylab("Proportion") +
  theme_minimal()
save_fig('gender', plot = g + ggtitle("Gender vs. Income"))
g
```
...
df$Nationality %>% table() %>% kable()
```

The table shows that the sample size from groups except United States is small, which may adversely affect modelling accuracy.
